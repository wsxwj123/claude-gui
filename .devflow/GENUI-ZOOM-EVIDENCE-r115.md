...
这张图的源码里 `Lip@V` 的 `@` 是 mermaid 11 的保留语法(边 id),**整张图第一次解析就失败**,
靠 `repairMermaidSource` 的重试才画出来;而该修复会把标签里的 `<br/>` 换成空格,
**作者写的换行点被丢掉**,折行位置改由 200px 自动断行决定。实测把标签加引号后保留 `<br/>` 可以按
作者意图断行(`verify-fix.mjs` 的乙组)。这属于"能更好"而非"显示不全",且要动 `repairMermaidSource`
的既有行为,留作待办。
